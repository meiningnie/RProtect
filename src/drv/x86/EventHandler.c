...
--*/
{
	...
	switch (ulCrimeType & CRIME_MAJOR_MASK) {
		...
		case CRIME_MAJOR_SYS:
			...
			{
				if (g_pProtected == (PEPROCESS)ulExtraInfo) {	// selfprotect
					return RP_STATUS_ERR;
				}
			}
			else if (ulCrimeType == CRIME_MINOR_NtOpenSection)
			{
				if (/*g_pPhysicalMemoryObj && */g_pPhysicalMemoryObj != (PVOID)ulExtraInfo)
					return RP_STATUS_OK;
			}
			break;
		default:
	...
	pEvtData = BuildEventData(pusCriminal, pusVictim, pCriminalEproc, pVictimEproc, ulCrimeType, ulExtraInfo);
	if (!pEvtData)
		return RP_STATUS_ERR;
...
--*/
{
	...
	pEvtData->ulExtraInfo = ulExtraInfo;
	...
	if (pusCriminal) {
		...
		wcscpy(pEvtData->usCriminal.Buffer, L"Unknown");
	...
	if (pusVictim) {
		...
		wcscpy(pEvtData->usVictim.Buffer, L"System-wide");
		pEvtData->usVictim.Length = 22;	// !!!!!
	} else {
		//wcscpy_s(pEvtData->usVictim.Buffer, MAX_PATH, L"Unknown");
		wcscpy(pEvtData->usVictim.Buffer, L"Unknown");
...
--*/
{
	...
	pUsrEvtData->ulEvtData = (ULONG)pEvtData;
	pUsrEvtData->ulCrimeType = pEvtData->ulCrimeType;
	pUsrEvtData->ulExtraInfo = pEvtData->ulExtraInfo;
