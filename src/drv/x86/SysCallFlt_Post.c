...
RPSTATUS PostFlt_NtOpenFile(PFLT_CONTEXT pFltContext, PHOOK_OBJECT pHookObj)
{
	...
	__try
	{
		PHANDLE FileHandle = (PHANDLE)pFltContext->ulXxx1;
		...
		ULONG OpenOptions = (ULONG)pFltContext->ulXxx6;
...
RPSTATUS PostFlt_NtOpenThread(PFLT_CONTEXT pFltContext, PHOOK_OBJECT pHookObj)
{
	...
	__try
	{
		PHANDLE ThreadHandle = (PHANDLE)pFltContext->ulXxx1;
		NTSTATUS NtStatus;
...
RPSTATUS PostFlt_NtOpenProcess(PFLT_CONTEXT pFltContext, PHOOK_OBJECT pHookObj)
{
	...
	__try
	{
		PHANDLE  ProcessHandle = (PHANDLE)pFltContext->ulXxx1;
...
RPSTATUS PostFlt_NtOpenSection(PFLT_CONTEXT pFltContext, PHOOK_OBJECT pHookObj)
{
	...
	__try
	{
		PHANDLE	SectionHandle = (PHANDLE)pFltContext->ulXxx1;
...
RPSTATUS PostFlt_NtReadVirtualMemory(PFLT_CONTEXT pFltContext, PHOOK_OBJECT pHookObj)
{
	...
	__try
	{
		...
		PVOID Buffer = (PVOID)pFltContext->ulXxx3;
		PULONG ReturnLength = (PULONG)pFltContext->ulXxx5;
...
RPSTATUS PostFlt_NtDuplicateObject(PFLT_CONTEXT pFltContext, PHOOK_OBJECT pHookObj)
{
	...
	__try
	{
		HANDLE SourceProcessHandle = (HANDLE)pFltContext->ulXxx1;
		HANDLE TargetProcessHandle = (HANDLE)pFltContext->ulXxx3;
		PHANDLE TargetHandle = (PHANDLE)pFltContext->ulXxx4;
